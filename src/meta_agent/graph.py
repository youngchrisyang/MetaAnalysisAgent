...
from pydantic import BaseModel, Field
 llama_index import download_loader
...
class GraphState(TypedDict):
    ...
    worklog: Annotated[Optional[str], operator.add]

def load_and_parse_pdf(state: GraphState) -> GraphState:
    document = get_llamaparsed_doc(state["paper_path"])
    return {"paper_content": document}

def extract_paper_meta_info(state: GraphState) -> GraphState:
...
def extract_sample_info(state: GraphState) -> GraphState:
    llm = OpenAI(temperature=0)
...
def extract_variable_info(state: GraphState) -> GraphState:
    llm = OpenAI(temperature=0)
...
def extract_variable_pair_info(state: GraphState) -> GraphState:
    llm = OpenAI(temperature=0)
...
def synthesize_meta_info(state: GraphState) -> GraphState:
    ...
    return {"worklog": current_worklog}

def create_meta_analysis_graph():
    workflow = StateGraph(GraphState)
    
    workflow.add_node("load_and_parse_pdf", load_and_parse_pdf)
    workflow.add_node("extract_paper_meta_info", extract_paper_meta_info)
    workflow.add_node("extract_sample_info", extract_sample_info)
    workflow.add_node("extract_variable_info", extract_variable_info)
    workflow.add_node("extract_variable_pair_info", extract_variable_pair_info)
    workflow.add_node("synthesize_meta_info", synthesize_meta_info)
...
if __name__ == "__main__":
    ...
    graph = create_meta_analysis_graph()
    output = graph.invoke({"content": pdf_path})
